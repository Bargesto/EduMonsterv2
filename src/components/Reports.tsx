...
export default function Reports() {
  ...
  const getChartData = () => {
    ...
    const labels = Array.from({ length: 7 }, (_, i) => {
      return format(subDays(new Date(), i), 'd MMMM', { locale: tr });
    }).reverse();
    ...
    if (viewMode === 'student') {
      ...
      const data = labels.map(label => {
        const dayPoints = student.pointHistory?.filter(history =>
          format(new Date(history.date), 'd MMMM', { locale: tr }) === label
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: viewMode === 'student' ? 'Öğrenci Puan Gelişimi' : 'Sınıf Puan Gelişimi',
      },
    },
    scales: {
      y: {
  ...
  return (
    <div className="container mx-auto px-4">
      <h1 className="text-2xl font-bold text-gray-900 mb-6">Puan Raporları</h1>
      ...
      {chartData ? (
        ...
        <div className="text-center text-gray-500 py-8">
          Seçili {viewMode === 'student' ? 'öğrenci' : 'sınıf'} için veri bulunamadı.
        </div>
      )}
    </div>
  );
